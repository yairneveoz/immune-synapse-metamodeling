...
%% read data:
% read max_diff array:
rTCR_max_diff_heatmap = dlmread('rTCR_max_diff_array.m');

% read TCR historgrams 3D array:
TCR_hist_model3_3D = load('TCR_hist_model3');
TCR_hist_model3 = ...
    TCR_hist_model3_3D.dep_decay_TCR_normalized_counts;

% read TCR historgrams 3D array:
pTCR_hist_model3_3D = load('pTCR_hist_model3');
pTCR_hist_model3 = ...
...
dep_values = 0:50:750; % nm
dec_values = 0:25:275; % nm
...
hold off
colorbar
title('Max slope of pTCR hist')
xlabel('Decay length (nm)')
ylabel('Depletion (nm)')
xticks(2:2:length(dec_values))
yticks(2:2:length(dep_values))
xticklabels(25*[1:2:length(dec_values)])
yticklabels(50*[1:2:length(dep_values)])
% xt
axis equal
axis tight
